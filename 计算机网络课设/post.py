...
def test():
    ...
    print(action)
    if action=='sentmail':
        username = base64.b64encode(data['username']['_value'].encode('utf-8')).decode('utf-8')
        password = base64.b64encode(data['password']['_value'].encode('utf-8')).decode('utf-8')
        print(data['receiver']['_value'])
        print(data['header']['_value'])
        print(data['content']['_value'])
        send(username,password,data['sender']['_value'],data['receiver']['_value'],data['header']['_value'],data['content']['_value'])
        return jsonify('发送成功！！！')
    if action=='getcontent':
        ...
        recive()
        print(temp)
        return jsonify({"header": temp[0], "content": temp[1].decode('utf-8')})
    return jsonify('发送失败！！！')


# y邮箱服务商
def send(username,password,sender,receiver,header,content):
...
def recive():
    ...
    # start_index = len(mails) - n  # 要获取的邮件数量
    # end_index = len(mails)  # 最新一封邮件的索引
    #
