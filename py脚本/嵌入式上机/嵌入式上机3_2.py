...
class World():
    '''This will hold the platforms and the goal. In this game, the world moves left and right rather than the player.'''
...
class Doom():
    ...
    def collided(self, player_rect):
        '''check if the player is currently in contact with any of the doom.shrink the rectangle for the fireballs to make it fairer'''
        for fireball in self.fireballs:
...
'''move the fireball the appropriate distance down the screen.Fireballs don't accellerate with gravity, but have a random speed.If the fireball has reached the bottom of the screen, regenerate it'''
...
def Winner():
    ...
    WinnerSurf = WinnerFont.render('You Win !', True, greyColour)
    WinnerRect = WinnerSurf.get_rect()
    WinnerRect.midtop = (320, 10)
...
background_image = "parallax-mountain-bg.png"
jump_sound = "Bandari.wav"
fireball_image = "Fiyah.png"
level = [
    "                              ",
    "                              ",
    "                              ",
